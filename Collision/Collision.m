...
classdef Collision
    properties
        planeNormal   % Property to store the normal vector of the tabletop plane
        planePoint    % Property to store a point on the tabletop plane
    end

    methods
        function self = Collision()
            % Constructor: Define the tabletop plane with a normal vector and point
            self.planeNormal = [0, 0, 1]; % Assuming the plane is flat on the table (y-plane)
            self.planePoint = [0, 0.310, 0]; % Adjust according to the height of your table
        end
        ...
        function plotPlane(self)
            ...
            Z = repmat(self.planePoint(2), size(X, 1), size(X, 2)); % Set Y as the table height
